...
from ._comm_hooks import *
from .dataset import LyqDataset
...
class LyqLab():
    ...
    def _check_model(
        ...
        self._model_dir: str = output_dir() + self._model_id + '/'
    ...
    def _check_main_model(self) -> None:
        ...
        检查main模型
        """
        info(f"检查main模型{self._model_id}文件中...")
        ...
        info(f"检查main模型{self._model_id}文件完成！")
    
    @master_function
    def _check_lab_dir(self) -> None:
        info(f"检查实验{self.id}文件夹是否存在中...")
    ...
    def _init_where(self):
        info("检查是否存在检查点文件中...")
        p = find_checkpoint(self._lab_dir)
        self._step: int = 0 # 记录模型执行的步数，0代表执行了0步，表明为初始状态
        if p is None:
            info("没有发现检查点文件，使用main初始模型构建！")
            self._where = self._main_dir
            self._step = 0
        else:
            info(f"发现检查点{p.name}，检查有效性中...")
            is_valid = True
            if not is_files_all_exists(str(p)+'/', self._checkpoint_files):
                is_valid = False
            
            if is_valid:
                info(f"检查点{p.name}有效，使用此检查点构建！")
                self._where = str(p)
                self._step = int(p.name[-5:])
            else:
                info(f"检查点{p.name}无效，使用main初始模型构建！")
                self._where = self._main_dir
                self._step = 0
        
        info(f"检查{self._lab_trace_file}中的步数是否合法...")
    ...
    def _prepare_data(self):
        ...
        self._dataset = load_dataset(
            ...
            data_files={
                "train": train_file(),
                "valid": small_valid_file()
            },
            streaming=True
        )
        self._start = LyqDataLoader.step_to_start(
            self._step,
            self._micro_batch_size_per_device * self._micro_num_per_batch,
            self._world_size
        )
        info(f"训练集跳过{self._start}条数据！")
        self._train_dataset = self._dataset['train'].skip(self._start).map(
        ...
        self._valid_dataset = self._dataset['valid']
        self._data_collator = DataCollatorForLanguageModeling(
            self._tokenizer,
            mlm=False
        )
        self._train_dataloader = LyqDataLoader(
            self._train_dataset,
            self._data_collator,
            self._micro_batch_size_per_device * self._micro_num_per_batch,
            self._world_size,
        ...
        self._train_dataloader_iter: LyqDataLoaderIterator = iter(self._train_dataloader)

        # self._valid_dataloader = LyqDataLoader(
        #     self._train_dataset,
        #     self._data_collator,
        #     self._lab.micro_batch_size_per_device * self._lab.micro_num_per_batch,
        #     start=0,
        #     end=1024,
        # )
        info("训练数据加载器和验证数据加载器构建完成！")
    ...
    def _pre_step(self) -> None:
        ...
        self.batch_data: BatchSchema = { # type: ignore
    ...
    def _push_trace(self) -> None:
        trace: TraceStepSchema = {
            ...
            'grad_sync_total_time': _commmetrix.total_time,
            'grad_sync_comp_time': _commmetrix.comp_time,
            'grad_sync_comm_time': _commmetrix.comm_time,
            'grad_sync_comm_bytes': _commmetrix.comm_bytes,
...
# def set_grad_sync_total_time(grad_sync_total_time: float) -> None:
#     global _current_lab
#     assert not _current_lab, "目前没有实验注册！"
#     _current_lab[0].grad_sync_total_time = grad_sync_total_time

# def set_grad_sync_comp_time(grad_sync_comp_time: float) -> None:
#     global _current_lab
#     assert not _current_lab, "目前没有实验注册！"
#     info(f"set_grad_sync_comp_time here!")
#     _current_lab[0].grad_sync_comp_time = grad_sync_comp_time

# def set_grad_sync_comm_time(grad_sync_comm_time: float) -> None:
#     global _current_lab
#     assert not _current_lab, "目前没有实验注册！"
#     _current_lab[0].grad_sync_comm_time = grad_sync_comm_time

# def set_grad_sync_comm_bytes(grad_sync_comm_bytes: int) -> None:
#     global _current_lab
#     assert not _current_lab, "目前没有实验注册！"
#     info(f"set_grad_sync_comm_bytes here!")
#     _current_lab[0].grad_sync_comm_bytes = grad_sync_comm_bytes